...
class Plugin:
    ...
    def set_params(self, **kwargs):
        """
        Actualiza los parámetros del endpoint combinando los parámetros específicos con la configuración global.
        """
    ...
    def get_debug_info(self):
        """
        Devuelve información de debug de los parámetros relevantes del endpoint.
        """
    ...
    def _handle_prediction_request(self):
        ...
        try:
            ...
            required_fields = ['model_path', 'target_date']
            ...
            num_batches = data.get('num_batches', self.params['default_num_batches'])
    ...
    def _process_prediction(self, request_id):
        ...
        try:
            ...
            predictor_plugin.load_model(prediction_request.model_path)
            
            # Get input features from request or use default
            input_features = []
            if prediction_request.input_features:
                input_features = json.loads(prediction_request.input_features)
            
            # Configure feeder with request parameters
            feeder_config = {
                'target_date': prediction_request.target_date,
                'batch_size': prediction_request.batch_size,
                'num_batches': prediction_request.num_batches,
                'input_features': input_features
            }
            feeder_plugin.set_params(**feeder_config)
            
            # Download required data using feeder plugin
            print(f"Downloading data for date {prediction_request.target_date}")
            data = feeder_plugin.fetch()
            
            if data is None or data.empty:
                raise ValueError("No data available for the specified date and parameters")
            
            print(f"Downloaded data shape: {data.shape}")
            
            # Make predictions
            print("Making predictions...")
            predictions = predictor_plugin.predict(data)
            
            # Format results
            results = {
                'predictions': predictions.tolist() if hasattr(predictions, 'tolist') else predictions,
                'target_date': prediction_request.target_date.isoformat(),
